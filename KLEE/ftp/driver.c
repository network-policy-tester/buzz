...
locatedPacket packetParser(char* pktStr){
	...
	pkt.packet.isFtp = atoi(a[18]);
	pkt.packet.PortM = atoi(a[19]);
	pkt.packet.isPortMRes = atoi(a[20]);
	pkt.packet.isPASV = atoi(a[21]);
	pkt.packet.isDatachannel = atoi(a[22]);
	pkt.packet.isDatachannelRes = atoi(a[23]);
...
// active mode
locatedPacket ftpproxyProc(int proxyIndex, locatedPacket inPkt){
	locatedPacket outPkt;
	outPkt = inPkt;
...
locatedPacket firewallftpProc(int fwIndex, locatedPacket inPkt){
	//first we set the outpacket to be the same as in packet
	locatedPacket outPkt;
	outPkt = inPkt;	
	...
	outPkt.port.num = fwPorts[fwIndex];
        ...
        //if (fwConnStates[fwIndex][inPkt.packet.connId] == NUL_FW){
                ...
                if (inPkt.packet.isDatachannel && inPkt.packet.fromClient){
